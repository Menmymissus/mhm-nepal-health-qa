...
//   if (event.key === " ") {
//     const currentWord = event.target.value.trim().split(" ").pop();

//     // Transliterate the word
//     const transliteratedWord = translateWords(currentWord).trim();

//     // Update the input field
//     event.target.value = event.target.value.replace(currentWord, transliteratedWord);
...
// });
  ...
  // console.log(sent)
     ...
     for (let i = 0; i < words.length; i++) {
         ...
         if (sW["smartconverter_on"] == "true") {
            console.log("smart converter on");
             if (hasSW(words[i]));
 ...
 export const transliterateToNepali = (englishText) => {
    // Call your existing translation logic function (translateWords)
    const transliteratedText = translateWords(englishText);
